...
window.openCheckout = async (courseId) => {
  ...
  document.getElementById('applyVoucherBtn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const checkoutResult = await createCheckout(courseId, 'course', code, { firstName: currentUser.name?.split(' ')[0] || 'Student', lastName: currentUser.name?.split(' ')[1] || '', email: currentUser.email, phone: '', country: 'Zimbabwe' });
