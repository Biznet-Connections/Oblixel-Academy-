...
console.log('🚀 Loading Part 2 (All Pages, AI Teacher, Final Exam Fix, Professor Card Position Fix, Admin Reorder, Add Admin, Like/Dislike, Certificate Codes, Single Price, Fixed Admin Panel, Modal Confirmations, Payment Type Fix)');
...
window.startExam = async (courseId) => {
  ...
  try {
    ...
    if (!response.ok) { throw new Error(examData.error || examData.message || 'Failed to start exam'); }
...
window.openCheckout = async (courseId) => {
  ...
  root.innerHTML = `
    <div class="max-w-4xl mx-auto p-4 fade-in">
      ...
      <div class="glass rounded-3xl p-6"><h1 class="text-2xl font-bold mb-6">Checkout</h1>
        <div class="glass rounded-2xl p-5 mb-6"><h2 class="text-xl font-bold mb-4">Order Summary</h2>
          <div class="space-y-3">
        ...
        <div class="glass rounded-2xl p-5"><h2 class="text-xl font-bold mb-4">💳 Payment Methods (Coming Soon)</h2>
          <div class="space-y-2 mb-4"><label class="flex items-center gap-3 glass p-3 rounded-xl cursor-not-allowed opacity-60"><input type="radio" disabled><span>💳 Credit Card <span class="text-xs text-yellow-400 ml-2">Coming Soon</span></span></label><label class="flex items-center gap-3 glass p-3 rounded-xl cursor-not-allowed opacity-60"><input type="radio" disabled><span>🅿️ PayPal <span class="text-xs text-yellow-400 ml-2">Coming Soon</span></span></label><label class="flex items-center gap-3 glass p-3 rounded-xl cursor-not-allowed opacity-60"><input type="radio" disabled><span>📱 EcoCash <span class="text-xs text-yellow-400 ml-2">Coming Soon</span></span></label></div>
          <div class="p-3 bg-yellow-500/10 rounded-xl text-xs text-yellow-400 mb-4"><i class="fa-solid fa-circle-info mr-1"></i> <strong>Vouchers = instant enrollment!</strong> Enter a voucher code above and click Apply.</div>
          <div id="autoEnrollStatus" class="hidden text-center p-4 bg-green-500/10 rounded-xl"><p class="text-green-400 font-bold text-lg">🎉 Enrolled Successfully!</p><p class="text-sm text-gray-300 mt-1">Redirecting to your course dashboard...</p><div class="thinking-dots mt-2"><span></span><span></span><span></span></div></div>
        </div>
      </div>
    </div>`;
...
async function renderAdmin() {
  ...
  function renderAdminContent() {
    ...
    for (var i = 0; i < currentCourses.length; i++) {
      ...
      html += '<tr class="border-b border-white/10 hover:bg-white/5 course-drag-row" draggable="true" data-course-id="' + c.id + '" data-display-order="' + i + '">';
      html += '<td class="py-3"><span class="drag-handle"><i class="fa-solid fa-grip-vertical"></i></span></td>';
      html += '<td class="py-3"><div class="flex items-center gap-2"><i class="fa-solid ' + (c.icon || 'fa-certificate') + ' text-purple-400"></i><span class="font-medium">' + escapeHtml(c.name) + '</span></div></td>';
      html += '<td>$' + cp + '</td><td>' + (c.enrolledCount || 0) + '</td>';
      html += '<td class="space-x-2"><button onclick="window.showEditCourseModal(\'' + c.id + '\')" class="text-cyan-400 hover:text-cyan-300"><i class="fa-solid fa-pen"></i></button><button onclick="window.showDeleteCourseModal(\'' + c.id + '\', \'' + escapeHtml(c.name).replace(/'/g, "\\'") + '\')" class="text-red-400 hover:text-red-300"><i class="fa-solid fa-trash"></i></button><button onclick="window.showManageVideosModal(\'' + c.id + '\')" class="text-yellow-400 hover:text-yellow-300"><i class="fa-solid fa-video"></i></button></td>';
      html += '</tr>';
    }
    ...
    if (vouchers.length > 0) {
      for (var j = 0; j < vouchers.length; j++) {
        var v = vouchers[j];
        html += '<tr class="border-b border-white/10 hover:bg-white/5"><td class="py-3"><span class="font-mono text-cyan-400">' + escapeHtml(v.code) + '</span><button onclick="window.copyToClipboard(\'' + v.code + '\')" class="ml-2 text-gray-400 hover:text-cyan-400"><i class="fa-regular fa-copy"></i></button></td>';
        html += '<td>' + (v.discountType === 'free' ? '🎁 FREE' : v.discountValue + (v.discountType === 'percentage' ? '% off' : '$ off')) + '</td>';
    ...
    if (users.length > 0) {
      for (var m = 0; m < users.length; m++) {
        var u = users[m];
        html += '<tr class="border-b border-white/10 hover:bg-white/5"><td class="py-3"><div><p class="font-medium">' + escapeHtml(u.name) + '</p><p class="text-xs text-gray-400">' + escapeHtml(u.email) + '</p></div></td>';
        html += '<td><span class="px-2 py-1 rounded-full text-xs ' + (u.role === 'admin' ? 'bg-purple-500/20 text-purple-400' : 'bg-gray-500/20') + '">' + u.role + '</span></td>';
        html += '<td>' + (u.enrolledCourses || 0) + '</td><td>$' + formatMoney(u.totalSpent || 0) + '</td>';
        html += '<td class="space-x-2">';
        if (u.role !== 'admin') { html += '<button onclick="window.makeUserAdminItem(\'' + u.id + '\')" class="text-yellow-400 hover:text-yellow-300"><i class="fa-solid fa-crown"></i> Make Admin</button>'; }
        if (u.email !== currentUser.email) { html += '<button onclick="window.showDeleteUserModal(\'' + u.id + '\', \'' + escapeHtml(u.name).replace(/'/g, "\\'") + '\')" class="text-red-400 hover:text-red-300 ml-2"><i class="fa-solid fa-trash"></i></button>'; }
        html += '</td></tr>';
      }
  ...
  setTimeout(function() {
    ...
    if (saveBtn) {
      saveBtn.addEventListener('click', async function() {
        ...
        var orderData = [];
        rows.forEach(function(row, index) { orderData.push({ courseId: row.dataset.courseId, displayOrder: index }); });
        try { await apiRequest('/admin/courses/reorder', { method: 'PUT', body: JSON.stringify({ courses: orderData }) }); showToast('Course order saved!', 'success'); await loadCourses(); window._adminCoursesData = coursesData.slice(); } catch (error) { showToast('Failed: ' + error.message, 'error'); }
      });
    }
...
function closeAIChatModal() { var modal = document.getElementById('aiChatModal'); if (modal) { modal.classList.add('hidden'); modal.classList.remove('flex'); } aiWidgetActive = false; aiChatHistory = [{ role: 'assistant', content: 'Hi! I\'m ObliXel AI, your study assistant. Ask me anything about our 25+ certifications, courses, exam prep, or vouchers! 🎓' }]; }
...
console.log('🎉 obliXel Academy v10.0 - COMPLETE - Payment Type Fixed');
